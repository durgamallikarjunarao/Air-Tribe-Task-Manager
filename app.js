...
app.get('/tasks/:id',(req,res)=>{
    ...
    if(allTasks.length ==0){
        ...
        return res.status(200).json(selectedTask[0]);
    }
});
...
app.put('/tasks/:id',(req,res)=>{
    ...
    if(fields_status.status==false){
        return res.status(400).send('Invalid attrributes from task');
    }
...
app.post('/tasks',(req, res)=>{
    ...
    fs.writeFile('./tasks.json',JSON.stringify(allTasks),{encoding:'utf-8',flag:'w'}, (err) =>{
        ...
        reload_task_manager();
        return res.status(201).send('successfully written new task into task list');
    });
});
